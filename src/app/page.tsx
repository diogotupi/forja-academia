...
export default async function LandingPage() {
  ...
  return <main><SiteHeader user={user} />
    ...
    <section className="catalog-preview" id="catalogo"><div><p className="eyebrow">LIDERFLIX ORIGINAL</p><h2>CONTEÚDO PARA AÇÃO.<br />NÃO PARA A ESTANTE.</h2><small className="carousel-hint">ARRASTE OU USE AS SETAS PARA EXPLORAR</small></div><ContentCarousel basePath={basePath} cards={[
      ...
      ["Inteligência Emocional Operacional","Controle antes da ação","team-overhead.webp"],
      ["Cultura de Alta Performance","Do discurso ao comportamento","resilience-team.webp"]
    ].map(([title,description,image])=>({title,description,image}))} /><Link className="button red" href={user ? "/catalogo" : "/cadastro"}>ACESSAR A PLATAFORMA</Link></section>
